...
class WalletGetPublicKeysTest : Spek({
    ...
    given("an Api.Wallet") {
        ...
        on("v1/wallet/get_public_keys") {

            val walletName = generateUniqueWalletName()

            walletApi.create(walletName).blockingGet()

            walletApi.createKey(listOf(walletName, "K1")).blockingGet()

            val keys = walletApi.getPublicKeys().blockingGet()

            it("should return the wallet password") {
                assertTrue(keys.body()!!.isNotEmpty())
            }
        }
    }
})