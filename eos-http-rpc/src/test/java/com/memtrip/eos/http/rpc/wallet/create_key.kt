...
class WalletCreateKeyTest : Spek({
    ...
    given("an Api.Wallet") {
        ...
        on("v1/wallet/create_key") {

            val walletName = generateUniqueWalletName()

            walletApi.create(walletName).blockingGet()

            val createKey = walletApi.createKey(listOf(walletName, "K1")).blockingGet()

            val getPublicKeys = walletApi.getPublicKeys().blockingGet().body()!!

            it("should return the public key") {
                assertNotNull(createKey.body())
                assertTrue(getPublicKeys.isNotEmpty())
                assertTrue(createKey.isSuccessful)
            }
        }
    }
})